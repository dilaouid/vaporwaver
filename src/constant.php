<?php

define('INCOMPLETE_KEYS', array('success' => 0, 'msg' => 'You need to complete the form!'));
define('NUMERIC_FONT', array('success' => 0, 'msg' => 'The font size should be a numeric value!'));
define('INCORRECT_FORMAT', array('success' => 0, 'msg' => 'The characters encoding is unknown!'));
define('FONT_SIZE_INCORRECT', array('success' => 0, 'msg' => 'The font size must be between 10 and 100!'));
define('INCORRECT_IMAGE', array('success' => 0, 'msg' => 'The uploaded image is incorrect!'));
define('INCORRECT_POSITION', array('success' => 0, 'msg' => 'The position is incorrect!'));
define('INCORRECT_COLOR', array('success' => 0, 'msg' => 'The color is incorrect!'));
define('INCORRECT_OVERLAY', array('success' => 0, 'msg' => 'The selected overlay does not exists!'));